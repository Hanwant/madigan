...
def make_data_dict(n):
    data = {
        "state_price": np.random.randn(n, obs_shape).astype(np.float32),
...
def make_data_tuple(n):
    return (
        ...
        np.random.randn(n, action_shape).astype(
            np.float32),  # actions for each asset
...
def add_RBP_dict(rbp, data):
    for i in range(len(data['state_price'])):
        sarsd = SARSD(
            State(data['state_price'][i], data['state_portfolio'][i], None),
            data['action'][i], data['reward'][i],
            State(data['next_state_price'][i], data['next_state_portfolio'][i],
                  None), data['done'][i])
        rbp.add(sarsd)
...
def add_RBP_tuple(rbp, data):
    for i in range(len(data[0])):
        sarsd = SARSD(State(data[0][i], data[1][i], None),
                      data[2][i], data[3][i],
                      State(data[4][i], data[5][i], None), data[6][i])
        rbp.add(sarsd)
...
def test_sampling_speed():
    ...
    time_profile(1000,
                 0,
                 py_sample=lambda: rbp.sample(32),
...
def test_sarsd_same(sample_py, sample_cy):
    for i, (a, b, name) in enumerate(
            zip([
                sample_py.state.price, sample_py.state.portfolio,
                sample_py.action, sample_py.reward, sample_py.next_state.price,
                sample_py.next_state.portfolio, sample_py.done
            ], [
                sample_cy['state_price'], sample_cy['state_portfolio'],
                sample_cy['action'], sample_cy['reward'],
                sample_cy['next_state_price'],
                sample_cy['next_state_portfolio'], sample_cy['done']
            ], [
                "state price", "state_portfolio", "action", "reward",
                "next_state_price", "next_state_portfolio", "done"
            ])):
        try:

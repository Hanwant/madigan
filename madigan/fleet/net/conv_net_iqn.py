...
class NormalHeadIQN(nn.Module):
    ...
    def forward(self, state_emb):
        qvals = self.out(state_emb).view(state_emb.shape[0], state_emb.shape[1],
...
class DuelingHeadIQN(nn.Module):
    ...
    def forward(self, state_emb):
        ...
        adv = self.adv_net(state_emb).view(state_emb.shape[0], state_emb.shape[1],
                                           self.n_assets, self.action_atoms)
...
class TauEmbedLayer(nn.Module):
    ...
    """
    def __init__(self, d_embed, d_model, device=None):
        super().__init__()

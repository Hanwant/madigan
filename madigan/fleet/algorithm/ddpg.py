...
class DDPG(OffPolicyActorCritic):
    ...
    def __init__(self, env, preprocessor, input_shape: tuple,
                 ...
                 test_steps: int, savepath: Union[Path, str], double_dqn: bool,
        ...
        self._action_space.transform = self.actions_to_transactions
        self.double_dqn = double_dqn
        self.discount = discount
        self.tau_soft_update = tau_soft_update
        output_shape = action_space.output_shape
        self.model_class_critic = get_model_class(
            type(self).__name__, model_class_critic)
        self.model_class_actor = get_model_class(
    ...
    def from_config(cls, config):
        ...
        action_space = ContinuousActionSpace(-1., 1., config.n_assets, 1)
        ...
        return cls(env, preprocessor, input_shape, action_space,
                   ...
                   savepath, aconf.double_dqn, aconf.tau_soft_update,
                   config.model_config.model_class_critic,
                   config.model_config.model_class_actor,
                   config.optim_config.lr_critic,
                   config.optim_config.lr_actor,
                   config.model_config)
    ...
    def to(self, device):
        ...
        self.critic_b.to(self.device)
        self.critic_t.to(self.device)
        self.actor_b.to(self.device)
    ...
    def actions_to_transactions(
        ...
        units = (desired_port - current_port) * self.env.equity
        return units
    ...
    #             "transaction to actions only implemented for ternary actions")
    #     return actions
    ...
    def get_qvals(self, state, action: torch.Tensor = None,
        ...
        if action is None:
            if target:
                action = self.actor_t(state)
            else:
                action = self.actor_b(state)
        if target:
    ...
    def __call__(self,
        ...
        qvals = self.get_qvals(state, action, target=target)
        return action, qvals
...
#         timestamp = torch.as_tensor(state.timestamp)
    ...
    def calculate_Gt_target(self, next_state, reward, done):
        ...
        """
        if self.double_dqn:
            behaviour_actions = self.critic_b(next_state).max(-1)[1]
            one_hot = F.one_hot(behaviour_actions,
                                self.action_atoms).to(self.device)
            greedy_qvals_next = (self.critic_t(next_state) * one_hot).sum(
                -1)  # (bs, n_assets, 1)
        else:
            greedy_qvals_next = self.critic_t(next_state).max(-1)[
                0]  # (bs, n_assets, 1)
    ...
    def save_state(self, branch=None):
        ...
        state = {
            'state_dict': self.critic_b.state_dict(),
    ...
    def load_state(self, branch=None):
        ...
        self.critic_b.load_state_dict(state['state_dict'])
        self.critic_t.load_state_dict(state['state_dict'])
